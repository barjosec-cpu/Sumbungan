...
Write-Host ""
Write-Host "=== Sumbungan CI/CD Auto (GitHub + Jenkins) ==="
Write-Host ""
...
if (-not $SkipJenkins) {
    if (-not (Wait-JenkinsReady)) {
        throw "Jenkins is not running at $JenkinsUrl - run scripts\start-jenkins.bat"
    }
}
...
if (-not $SkipGitHub -and -not $DeployOnly) {
    if (-not $GithubToken) {
        ...
        Write-Host "Pushing to GitHub ($GithubRepo branch $Branch)..."
...
if (-not $SkipJenkins) {
    if (-not $JenkinsToken) {
        Write-Warning "JENKINS_API_TOKEN not set in .env - run scripts\jenkins-create-api-token.bat"
        exit 1
    }
